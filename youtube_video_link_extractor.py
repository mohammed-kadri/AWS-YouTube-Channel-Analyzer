...
CHANNEL_ID = "UC5rUhLWmnTZWtdd8rI9xiCw"
...
def getChannelVideos(channelId, dateToStartFrom, dateToGoBackTo, timeInterval):
    ...
    while not done:
        if goBackTo < dateToGoBackTo:
            log.debug('The interval is now larger than the remaining time span to retrieve videos for. Using the date to go back to as the next boundary')
